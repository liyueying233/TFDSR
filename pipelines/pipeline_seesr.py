    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                # if (i+1) % 5 == 0:
                #     img_t = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
                #     img_t = self.image_processor.postprocess(img_t, output_type=output_type, do_denormalize=[True] * image.shape[0])
                #     folder = f'val/timesteps_{(i+1)//5}_sample'

                #     if not os.path.exists(folder):
                #         os.makedirs(folder)
                #     file_path = os.path.join(folder, f'{name}.png')

                #     img_t[0].save(file_path)
