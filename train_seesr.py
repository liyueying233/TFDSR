...
def print_model_stats(model, args):
    ...
    # 计算 FLOPs
    flops = torchprofile.profile_macs(model, args)
    # 计算参数量
...
if args.unet_model_name_or_path:
    ...
    logger.info(f"Loading unet weights from self-train: {args.unet_model_name_or_path}")
    ...
    logger.info(f"Loading unet weights from SD: {args.pretrained_model_name_or_path}")
    unet = UNet2DConditionModel.from_pretrained(
...
RAM = ram(pretrained='/mlx/users/liyueying.flora/playground/TFDSR/preset/models/ram_swin_large_14m.pth',
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(unet):
            ...
            if loss.isnan().any():
                if(model_pred.float().isnan().any()):
                    print('model_pred is nan')
                if(target.float().isnan().any()):
                    print('target is nan')
        ...
        # 计算参数量和FLOPs（仅在第一个step计算）
        if step == 0 and epoch == first_epoch:
            # 计算参数量
